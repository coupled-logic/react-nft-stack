...
import { useAdulam, BASE_URI, payForArt } from '../Adulam';

const Hero = () => {
  const { loading, error, connectedAccount, nfts, contract, loadWeb3, connectWallet, payForArt } = useAdulam();
  const {
    state: { maxSupply },
    // state: { connectedAccount, maxSupply, nfts },
    dispatch,
  } = useGlobalState();
